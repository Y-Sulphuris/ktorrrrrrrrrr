    ...
    routing {
        route("auth") {
            post("login") {
                ...
                emailQueue.send(EmailMessage("admin@example.com", "Кто-то залогинился", "Кто-то залогинился!!! (я не знаю зачем этот воркер но так надо)"))
...
//        authentication {
            route("products") {
                ...
                get("{id}") {
                    ...
                    val productId = try { UUID.fromString(id) } catch (e: Exception) { return@get call.respond(HttpStatusCode.BadRequest, "Invalid id") }
            ...
            route("orders") {
                post {
                    ...
                    val products = request.items.map { item ->
                        val product = productService.findProductById(UUID.fromString(item.productId))
                            ?: return@post call.respond(HttpStatusCode.BadRequest, "Product ${item.productId} not found")

                        if (product.price < item.count) {
                            return@post call.respond(HttpStatusCode.BadRequest, "Not enough stock for ${product.id}")
                        }
                        product
                    }
                    ...
                    emailQueue.send(EmailMessage("admin@example.com", "Кто-то создал ордер",
                ...
                delete("{id}") {
                    ...
                    val orderId = UUID.fromString(id) ?: return@delete call.respond(HttpStatusCode.BadRequest, "Invalid id")
